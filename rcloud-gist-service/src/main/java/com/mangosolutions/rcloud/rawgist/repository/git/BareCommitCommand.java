...
/**
 * Commits changes to a a bare git repository. This is based upon the JGit
 * CommitCommand
 *
 */
public class BareCommitCommand extends GitCommand<RevCommit> {
     ...
     *            the git repository
     * @param index
     *            the index file to use for the commit
     */
    ...
    public BareCommitCommand setAll(boolean all) {
        checkCallable();
        if (all && !only.isEmpty()) {
            throw new JGitInternalException(MessageFormat.format(JGitText.get().illegalCombinationOfArguments, "--all", //$NON-NLS-1$
                    "--only")); //$NON-NLS-1$
        }
        this.all = all;
        return this;
     ...
     *
     * @param amend
     *            true to amend the tip of the current branch
     * @return {@code this}
     */
    ...
    public BareCommitCommand setOnly(String only) {
        checkCallable();
        if (all) {
            throw new JGitInternalException(MessageFormat.format(JGitText.get().illegalCombinationOfArguments, "--only", //$NON-NLS-1$
                    "--all")); //$NON-NLS-1$
        }
