...
public class UserRestControllerTest {
    ...
    public void testGetUserWithApplicationJsonMediaType() throws Exception {
        ResultActions resultActions = mvc.perform(get("/user").accept(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(status().isOk()).andExpect(jsonPath("$.login", is("mock_user")));
    }

    @Test
    @WithMockUser("mock_user")
    public void testGetUserWithGithubBetaMediaType() throws Exception {
        ResultActions resultActions = mvc.perform(get("/user").accept(GITHUB_BETA_MEDIA_TYPE))
                .andExpect(status().isOk()).andExpect(jsonPath("$.login", is("mock_user")));
    }
    ...
    public void testGetUserWithGithubV3MediaType() throws Exception {
        ResultActions resultActions = mvc.perform(get("/user").accept(GITHUB_V3_MEDIA_TYPE)).andExpect(status().isOk())
