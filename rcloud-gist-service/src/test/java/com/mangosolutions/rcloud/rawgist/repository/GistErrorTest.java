...
public class GistErrorTest {
	...
	public void gistErrorFormattingNoParamsTest() {
		...
		GistError error = new GistError(GistErrorCode.ERR_COMMENT_NOT_EXIST, "This has a token, but should not be replaced {}");
		Assert.assertEquals("This has a token, but should not be replaced {}", error.getMessage());
		Assert.assertEquals("ERR_COMMENT_NOT_EXIST: This has a token, but should not be replaced {}", error.getFormattedMessage());
